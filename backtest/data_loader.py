...
def _synth_ohlcv(symbol: str, bar: str, days: int) -> pd.DataFrame:
    ...
    np.random.seed(seed)
    dt = minutes / (24 * 60)
    mu, sigma = (0.0002, 0.045) if symbol.upper() == "ETH" else (0.0003, 0.05)
    ...
    timestamps = pd.date_range(end=pd.Timestamp.utcnow().floor("min"), periods=n, freq=f"{minutes}min")
    return pd.DataFrame({
...
def _synth_fear_greed(days: int) -> pd.Series:
    ...
    dates = pd.date_range(end=pd.Timestamp.utcnow().date(), periods=days, freq="D").date
...
def _synth_timeseries(days: int, freq_hours: int, seed: int, base: float = 0.0, vol: float = 0.05) -> pd.Series:
    ...
    end = pd.Timestamp.utcnow().floor(f"{freq_hours}h")
...
def load_cmc_ohlcv(symbol: str = "ETH", bar: str = "5m", days: int = 90) -> pd.DataFrame:
    ...
    api_key = os.environ.get("CMC_API_KEY")
    if not api_key:
        print(f"[data_loader] No CMC_API_KEY — using synthetic {symbol} {bar} ({days}d). Set CMC_API_KEY for live data.")
